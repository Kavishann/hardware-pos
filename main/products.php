...
function sum() {
            ...
            if (!isNaN(result)) {
                var oresult = parseFloat(txtFirstNumberValue) - parseFloat(result);
                document.getElementById('txt55').value = oresult;
				 
				  }
        var txtFirstNumberValue = document.getElementById('txt2').value;
            var result = parseFloat(txtFirstNumberValue) *10/100;
            if (!isNaN(result)) {
                var oresult = parseFloat(txtFirstNumberValue) - parseFloat(result);
                document.getElementById('txt19').value = oresult;
				 
				  }
				  var txtFirstNumberValue = document.getElementById('txt19').value;
				  var txtsecondNumberValue = document.getElementById('txt55').value;
...
<div class="container-fluid">
	...
	<div class="span2">
          <div class="well sidebar-nav">
			...
			<li><a href="category.php"><i class="icon-plus icon-2x"></i> Categorys</a> </li>
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			<th width="12%"> Vehicle Name </th>
			<th width="11%"> Part Number </th>
			<th width="8%"> Category</th>
			<th width="12%"> Supplier </th>
			<th width="10%"> Date Received </th>
			<th width="3%"> C.Discount </th>
			<th width="5%"> Colombo Price </th>
			<th width="6%"> Bought Price </th>
...
$first="<a href=\"products.php?p=1\">First</a>";
#last
$lastpn=ceil($rowcount / $ppage);
$last = "<a href=\"products.php?p={$lastpn}\">Last</a>";
#next
if ($pagen >= $lastpn) {
	$next = "<a>next</a>";
} else{
    $npnum=$pagen + 1;
	$next = "<a href=\"products.php?p={$npnum}\">Next</a>";
}
#prev
if ($pagen <= 1) {
	$pre = "<a>previous</a>";
    ...
    $prepn=$pagen - 1;
	$pre = "<a href=\"products.php?p={$prepn}\">Previous</a>";
}


//end kavishan pagnition
				include('../connect.php');
				$result = $db->prepare("SELECT *, bprice * 	qty_sold as total FROM products ORDER BY product_id DESC LIMIT {$start},{$ppage}");
				$result->execute();
...
<?php
 echo $first ." | ".$pre." | "."Page ".$pagen." of ".$lastpn." ".$next." | ".$last;
 ?>
</div>
